...
module.exports.run = async ({ api, event, args }) => {
	...
	let z_1 = (hours < 10) ? '0' + hours : hours;
	let x_1 = (minutes < 10) ? '0' + minutes : minutes;
	let y_1 = (seconds < 10) ? '0' + seconds : seconds;
	...
	const timeNow = moment.tz("Asia/Karachi").format("DD/MM/YYYY || HH:mm:ss"); // Timezone set to Pakistan/Islamabad
	...
	if (args[0] == "list") {
		...
		let count = alime.listAnime.length;
		let data = alime.listAnime;
		let page = 1;
	...
	let id = k ? k : Math.floor(Math.random() * 848) + 1;
	...
	let avtAnime = (await axios.get(encodeURI(`${lengthchar[id].imgAnime}`), { responseType: "arraybuffer" })).data;
	...
	Canvas.registerFont(__dirname + `/tad/Grandstander-Bold.ttf`, { family: "Grandstander" });
